...
def _compute_topk_reverse_kl(
    ...
    if tail_bucket and strategy not in ("only_stu", "intersection"):
        # The k+1 partition is only exact when all student logprobs come from one
        # softmax; only_tch/union/union-intersection mix the rollout harvest with a
        # separate rescoring pass (also validated at startup).
        raise ValueError(f"--opd-topk-tail-bucket requires --opd-top-k-strategy only_stu or intersection, got {strategy!r}.")
    ...
    if len(teacher_responses) == 1:
        ...
        # top-k ids (strategy validated to only_stu at startup); mix raw teacher
        ...
        if strategy != "only_stu":
            raise ValueError(f"Teacher ensembles require --opd-top-k-strategy only_stu, got {strategy!r}.")
