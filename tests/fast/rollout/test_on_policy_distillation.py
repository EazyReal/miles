...
def test_topk_ensemble_uses_mixture_of_teachers():
    ...
    reverse_kl = _compute_topk_reverse_kl(_args("only_stu"), _sample(), payload)
...
def test_topk_ensemble_rejects_non_student_strategies():
    ...
    with pytest.raises(ValueError, match="require --opd-top-k-strategy only_stu"):
...
def _tail_args(strategy: str = "only_stu"):
    return Namespace(
        opd_top_k_strategy=strategy,
...
def test_topk_tail_bucket_rejects_mixed_source_strategies():
    # only_tch/union/union-intersection fill student logprobs from a separate
    # rescoring pass, so the k+1 bucket masses do not come from one softmax.
    with pytest.raises(ValueError, match="only_stu or intersection"):
